use anyhow::{anyhow, Error};
use log::{error, info, warn};
use once_cell::sync::Lazy;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::sync::Mutex;

#[derive(Serialize, Deserialize)]
struct Res {
    records: Vec<Record>,
}

#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct Record {
    pub id: String,
    pub name: String,
    pub value: String,
    pub updated_on: String,
    pub line_id: String,
}

struct DnspodConfig {
    token: String,
    domain: String,
    sub_domain: String,
    ip_url: String,
}

static CONFIG: Lazy<Mutex<Option<DnspodConfig>>> = Lazy::new(|| Mutex::new(None));

/// 初始化DNSPod配置
pub fn init(
    token: String,
    domain: String,
    sub_domain: String,
    ip_url: Option<String>,
) -> Result<(), Error> {
    let ip_url = ip_url.unwrap_or("http://whatismyip.akamai.com".to_string());
    info!(
        "初始化DNSPod配置: monitor current ip by [{}] and modify [{}.{}] with token [{}]",
        ip_url, sub_domain, domain, token
    );

    let config = DnspodConfig {
        token,
        domain,
        sub_domain,
        ip_url,
    };

    let mut cfg = CONFIG
        .lock()
        .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
    *cfg = Some(config);

    Ok(())
}

/// 获取当前IP地址
pub fn current_ip() -> Result<String, Error> {
    let cfg = CONFIG
        .lock()
        .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
    let config = cfg
        .as_ref()
        .ok_or_else(|| anyhow!("Config not initialized"))?;

    let result = reqwest::blocking::get(&config.ip_url);
    match result {
        Ok(ip) => match ip.text() {
            Ok(text) => Ok(text),
            Err(e) => Err(anyhow!(e)),
        },
        Err(e) => Err(anyhow!(e)),
    }
}

/// 获取DNS记录
pub fn get_record() -> Result<Option<Record>, Error> {
    let cfg = CONFIG
        .lock()
        .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
    let config = cfg
        .as_ref()
        .ok_or_else(|| anyhow!("Config not initialized"))?;

    let mut params: HashMap<&'static str, &str> = HashMap::new();
    params.insert("login_token", &config.token);
    params.insert("format", "json");
    params.insert("error_on_empty", "no");
    params.insert("lang", "en");
    params.insert("domain", &config.domain);
    params.insert("sub_domain", &config.sub_domain);

    let client = reqwest::blocking::Client::new();
    let res = client
        .post("https://dnsapi.cn/Record.List")
        .form(&params)
        .send();
    let text = res?.text()?;
    let result: serde_json::Result<Res> = serde_json::from_str(&text);
    match result {
        Ok(res) => {
            if !res.records.is_empty() {
                info!("current record is {:?}", res.records[0]);
                Ok(Some(res.records[0].clone()))
            } else {
                Ok(None)
            }
        }
        Err(err) => {
            warn!("error parse result: {}", text);
            Err(anyhow!(err))
        }
    }
}

/// 修改DNS记录
pub fn modify_record(current_ip: &String, record: &Record) -> Result<(), Error> {
    let cfg = CONFIG
        .lock()
        .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
    let config = cfg
        .as_ref()
        .ok_or_else(|| anyhow!("Config not initialized"))?;

    if &record.value != current_ip {
        let client = reqwest::blocking::Client::new();
        let mut params: HashMap<&'static str, &str> = HashMap::new();
        params.insert("login_token", &config.token);
        params.insert("format", "json");
        params.insert("error_on_empty", "no");
        params.insert("lang", "en");
        params.insert("domain", &config.domain);
        params.insert("sub_domain", &record.name);
        params.insert("record_id", &record.id);
        params.insert("record_line_id", &record.line_id);
        params.insert("value", current_ip);
        let res = client
            .post("https://dnsapi.cn/Record.Ddns")
            .form(&params)
            .send();
        if let Ok(res) = res {
            let text = res.text();
            if let Ok(text) = text {
                info!("modify result is： {}", text);
            }
        } else {
            info!("error modify record");
            return Err(anyhow!("Error modify record"));
        }
    }
    Ok(())
}

/// 添加DNS记录
pub fn add_record(current_ip: &str) -> Result<(), Error> {
    let cfg = CONFIG
        .lock()
        .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
    let config = cfg
        .as_ref()
        .ok_or_else(|| anyhow!("Config not initialized"))?;

    let client = reqwest::blocking::Client::new();
    let mut params: HashMap<&'static str, &str> = HashMap::new();
    params.insert("login_token", &config.token);
    params.insert("format", "json");
    params.insert("error_on_empty", "no");
    params.insert("lang", "en");
    params.insert("domain", &config.domain);
    params.insert("sub_domain", &config.sub_domain);
    params.insert("record_type", "A");
    params.insert("record_line", "默认");
    params.insert("value", current_ip);
    let res = client
        .post("https://dnsapi.cn/Record.Create")
        .form(&params)
        .send();
    if let Ok(res) = res {
        let text = res.text();
        if let Ok(text) = text {
            info!("add result is： {}", text);
            return Ok(());
        }
    }

    info!("error add record");
    Err(anyhow!("Error adding record"))
}

/// 更新DNS记录（包括检查、添加或修改记录）
pub fn update_dns_record(current_ip: &String) -> Result<(), Error> {
    match get_record() {
        Ok(Some(record)) => {
            if current_ip != &record.value {
                info!("ip changed from {} to {}", record.value, current_ip);
                modify_record(current_ip, &record)?;
            } else {
                info!("ip not changed");
            }
        }
        Ok(None) => {
            let cfg = CONFIG
                .lock()
                .map_err(|_| anyhow!("Failed to lock CONFIG"))?;
            let config = cfg
                .as_ref()
                .ok_or_else(|| anyhow!("Config not initialized"))?;

            info!("no such record: {}.{}", config.sub_domain, config.domain);
            add_record(current_ip)?;
        }
        Err(e) => {
            warn!("error get record: {}", e);
            return Err(e);
        }
    }

    Ok(())
}
